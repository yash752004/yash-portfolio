              ...
              >
                ...
                <div className="h-48 w-full overflow-hidden bg-slate-100/50 relative flex items-center justify-center p-6 border-b border-slate-100">
                  <img 
                    ...
                    alt={project.title} 
                    className="max-h-full max-w-full object-cover group-hover:scale-[1.05] transition-transform duration-500 ease-out" 
                  />
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-100 space-y-4">
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {(project.tools?.length || 0) > 4 && (
                         <span className="text-[9px] font-extrabold text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-200/50">+{project.tools.length - 4}</span>
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div key={project.id} className="bg-white rounded-[2rem] w-full max-w-6xl max-h-[90vh] overflow-y-auto shadow-2xl relative flex flex-col [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full">
              ...
              <div className="w-full h-[35vh] md:h-[45vh] bg-slate-50 relative flex items-center justify-center overflow-hidden border-b border-slate-100">
                 ...
                 <div className="absolute inset-0 z-0">
                   <img src={project.thumbnail} alt="Background blur" className="w-full h-full object-cover blur-2xl opacity-30 scale-110" />
                 </div>
                 {/* Hero Image */}
                 <img src={project.thumbnail} alt={project.title} className="max-w-[90%] max-h-[85%] object-contain relative z-10 drop-shadow-2xl rounded-2xl transition-transform duration-700 hover:scale-105" />
              </div>
                 ...
                 <div className="lg:w-1/3 flex flex-col gap-8 lg:sticky lg:top-8 h-fit">
                    ...
                    {project.metricsEnabled && (
                      <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200/60 rounded-3xl relative overflow-hidden group">
                        ...
                        <span className="text-3xl font-black text-slate-800 block relative z-10 mb-1">{project.metricsValue || "+140% Growth"}</span>
                        <span className="text-sm text-slate-500 font-medium relative z-10">{project.metricsLabel || "Operations scale"}</span>
                      </div>
                    )}
                    ...
                    {project.keyCapabilities && project.keyCapabilities.length > 0 && (
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          {project.keyCapabilities.map((feat, i) => (
