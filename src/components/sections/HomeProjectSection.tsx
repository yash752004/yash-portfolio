import { getProjectDatas, type ProjectDetailType } from "@/projects/projectDetails";
...
const HomeProjectSection = () => {
  ...
  return (
    <section className="page-section pt-0">
      ...
      <div className="container mt-16 md:mt-20 flex flex-col md:flex-row gap-0 items-stretch rounded-2xl overflow-clip min-h-[400px]">
        ...
        <div className="w-full md:w-5/8 flex flex-col md:flex-row gap-16 md:gap-0 rounded-2xl overflow-clip items-stretch">
          {homeProjects.map((proj, index) => (
            <div
              key={proj.id}
              className={`relative shadow-lg overflow-hidden min-h-95 w-full md:w-1/5 md:hover:w-[40%] rounded-2xl md:rounded-none transition-all duration-150 group cursor-pointer`}
              onClick={(e) => navigate(`/projects#${proj.id}`)}
              role='button'
              tabIndex={0}
            >
              <img
                src={resolveImagePath(proj.verticalThumbnail)}
                alt={proj.title}
